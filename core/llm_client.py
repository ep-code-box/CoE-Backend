...
_clients: Dict[str, AsyncOpenAI] = {} # Changed type hint

def _create_client_for_provider(model_info: ModelInfo) -> AsyncOpenAI: # Changed return type hint
    """프로바이더별 OpenAI 클라이언트를 생성합니다."""
    provider = model_info.provider
    if provider == "sktax":
        return AsyncOpenAI( # Changed instantiation
...
def get_client_for_model(model_id: str) -> AsyncOpenAI: # Changed return type hint
...
langchain_client = ChatOpenAI(
    ...
    streaming=True,
    openai_api_base=str(client.base_url),  # base_url을 명시적으로 전달
