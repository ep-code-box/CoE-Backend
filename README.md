...
*   **AI 에이전트 채팅**: OpenAI 호환 채팅 API (`/v1/chat/completions`)
*   **코딩 어시스턴트**: 코드 생성, 분석, 리팩토링, 리뷰 (`/api/coding-assistant/`)
*   **RAG/임베딩**: `CoE-RagPipeline` 연동을 통한 Git 소스코드/RDB 스키마 분석 및 벡터 검색
*   **LangFlow 연동**: 외부 LangFlow 서버를 통한 워크플로우 관리 (`/flows/`)
*   **동적 도구**: 자동 도구 등록 및 관리
...
*   **`RAG_PIPELINE_URL`**: CoE-RagPipeline 서비스의 URL (예: `http://localhost:8001`)
...
추가 요구 사항.
- langFlow로 부터 json을 받아 api로 노출한다.
- 임베딩 관련은 모두 CoE-RagPipeline쪽 엔드포인트를 사용한다.
- langFlow로 부터 요청 받는 리퀘스트 내용에는 end-point/description/flow-body/flow-id 들이 있다.
- langflow가 추가되면 동적으로 타 시스템에서 연동가능하게 노출한다. 
- db에 저장관리하며. 주기적으로 db 내용을 확인해 전체 노출하도록 한다.
- 하나의 파일은 300라인 넘지 않도록 모듈화를 한다.
