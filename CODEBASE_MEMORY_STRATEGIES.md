...
## 5. 구현된 아키텍처: '서기 에이전트'를 통한 동적 지식 축적 (Hybrid RAG)

현재 프로젝트는 정적 문서화와 동적 학습을 결합한 하이브리드 RAG 접근 방식을 채택하고 있습니다. 핵심은 '서기(Scribe) 에이전트'라는 개념을 도입하여, 대화 속에서 자연스럽게 발생하는 설계 결정과 규칙을 자동으로 포착하고 축적하는 것입니다.

### 핵심 개념
*   **메인 에이전트**는 사용자와의 대화에 집중합니다.
*   별도의 **서기 에이전트**가 백그라운드에서 모든 대화 내용을 실시간으로 분석합니다.
*   서기 에이전트는 대화에서 장기적으로 기억해야 할 지식(설계 규칙, 개발 패턴, 히스토리 등)을 식별합니다.
*   식별된 지식은 중앙 지식 저장소(예: `docs/architecture/accumulated_knowledge.md`)에 자동으로 기록됩니다.
*   이 지식 저장소는 RAG 시스템에 의해 인덱싱되어, 추후 모든 대화에서 AI 에이전트가 참고하는 '기억'으로 활용됩니다.

### 장점
*   **자동화된 지식 축적**: 개발자가 별도의 문서화 작업을 하지 않아도, 대화만으로 지식 베이스가 자연스럽게 성장합니다.
*   **낮은 오버헤드**: 지식 축적 과정이 백그라운드에서 비동기적으로 처리되므로 메인 대화의 성능에 영향을 주지 않습니다.
*   **실용성**: 정적인 문서(ADRs, 개발 가이드)와 동적인 대화 기반 지식을 모두 활용하여 기억의 폭과 깊이를 더합니다.

### 단점 및 고려사항
*   **지식 식별의 정확성**: 서기 에이전트가 저장할 가치가 있는 정보를 얼마나 정확하게 식별하는지에 따라 시스템의 효율성이 크게 좌우됩니다.
*   **지식 검증**: 잘못된 정보나 불필요한 내용이 저장될 경우를 대비해, 축적된 지식을 주기적으로 검토하고 수정하는 프로세스가 필요합니다.

---

## 결론

본 프로젝트에서는 **'서기 에이전트'를 통해 동적으로 지식을 축적하는 하이브리드 RAG 방식**을 초기 아키텍처로 구현했습니다. 이는 정적 RAG가 가진 장점을 취하면서, 대화의 흐름 속에서 자연스럽게 지식을 최신으로 유지하는 것을 목표로 합니다.

앞으로 RAG의 인덱싱 품질을 높이는 전략(의미 기반 청킹, 메타데이터 강화)을 적용하는 것이 중요합니다.
